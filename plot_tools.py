...
def plot_confusion(true_labels, predictions, classes, title="Confusion Matrix"):
    ...
    matplotlib.pyplot.xticks(tick_marks, classes, rotation=45, fontsize=12)
    matplotlib.pyplot.yticks(tick_marks, classes, fontsize=12)
    ...
    matplotlib.pyplot.ylabel('True label', fontsize=15)
    matplotlib.pyplot.xlabel('Predicted label', fontsize=15)
...
def plot_learning(features_matrix, labels, classifier=XGBClassifier(), n_splits=10, 
    ...
    matplotlib.pyplot.ylabel("Score (%)", fontsize=15)
    matplotlib.pyplot.title(title, fontsize=18)
